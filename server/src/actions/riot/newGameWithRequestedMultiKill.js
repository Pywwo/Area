...
const name = "new_game_with_requested_multi_kills";

const checkNewGameWithRequestedMultiKills = (action, callback) => {
    let apiKey = process.env.RIOT_API_KEY;
    let sumId = "0";
    const multiKill = ["a kill", "a double kill", "a triple kill", "a quadra kill", "a PENTAKILL !!"];
    request.get({
    headers: {
        "X-Riot-Token": apiKey,
    },
            ...
            request.get({
                ...
                pl.forEach(item => {
                    if (item.participantId === playerId) {
                        if (item.stats.largestMultiKill >= action.params.multi_kill) {
                            const outputs = {
                                multi_kill_asked : multiKill[action.params.kills_asked - 1],
...
const trigger = (action, callback) => {
    ...
    checkNewGameWithRequestedMultiKills(action, (should, outputs) => {
        if (!should)
            callback(false);
        else
            callback(true, outputs);
